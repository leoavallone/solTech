...
export default function PaginaCobranca() {
    ...
    const getInvoice = async () => {
        const { data: invoices } = await apiClient.get('/invoice/list', {
            headers: {
                'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiIDogImFjY291bnRzLm9jcHAtY3NzLmNvbSIsICJhdWQiIDogIndlYi1vY3BwLWNzcy5jb20iLCAic3ViIiA6ICIwNTE4NmU5Yjg0MjU0MmU0NmRlOTZkZjdlNmRmOGM5MTc1NGY4NzYxIiwgImlhdCIgOiAxNzU5NDMyMzczLCAiZXhwIiA6IDE3NTk0MzU5NzN9.P5drcXlVruqIFzrjUhvb6jEoYWSp1U9wPjELvQtnLGw'
            }
        });
        if (invoices.length > 0) {
            let notList: ((prevState: Invoice[]) => Invoice[]) | { code: any; clientName: any; clientAlias: any; currency: any; currencyLabel: any;  amount: any; dateISO: any; state: any; }[] = []
            invoices.forEach((invoice: {
                data: any; code: any; clientname: any; clientcode: any; currencycode: any; currencyname: any; transaction: { amount: any; }; created: any; statelabel: any;
            }) => {
                notList.push({
                    code: invoice.code,
                    clientName: invoice.clientname,
                    clientAlias: invoice.clientcode,
                    currency: invoice.currencycode,
                    currencyLabel: invoice.currencyname,
                    amount: 0,
                    dateISO: invoice.created,
                    state: invoice.statelabel
                })

            });
            console.log(notList)
            setInvoiceList(notList)
        }
    }
    ...
    const filtered = useMemo(() => {
        ...
        let arr = invoiceList.filter((inv) => {
            if (!q) return true;
            return (
        ...
        return arr;
    }, [query, sortKey, sortDir, invoiceList]);
